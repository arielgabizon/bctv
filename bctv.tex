...
% thus someone generating parameters using the libsnark key generation algorithm (e.g. in a designated verifier setting) is not vulnerable to the attack. However, someone using parameters completely matching the \cite{BCTV} paper, e.g. ones generated in the MPC described in \cite{BGG}, is.
...
\item $e(\PI(x)+\pi_A,\pi_B)= e(\pi_C,g_2)\cdot e(\pi_H,\enc2{Z(\tau)\rho_A\rho_B})$.
 ...
 Note that the elements \sett{\pk'_{A,i}}{i\in [0..n]} are not used at all by the verifier and honest prover, and thus could have been omitted from the key. We show here these elements allow to replace the public input arbitrarily when starting from a valid proof.
